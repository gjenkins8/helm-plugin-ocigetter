...
func CertPoolFromFile(filename string) (*x509.CertPool, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("can't read CA file: %v: %q", filename, err)
	}
	cp := x509.NewCertPool()
	if !cp.AppendCertsFromPEM(b) {
		return nil, fmt.Errorf("failed to append certificates from file: %s: %q", filename, err)
	}
	return cp, nil
}
...
func CertFromFilePair(certFile, keyFile string) (*tls.Certificate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("can't load key pair from cert %s and key %s: %q", certFile, keyFile, err)
	}
	return &cert, err
}
